...
#define DEBUG_SERIAL_RX_BUF_SIZE 1024u

static serial_func_t s_tx_handler = 0;
static hdlc_parser_t s_parser;
static uint8_t s_rx_buf[DEBUG_SERIAL_RX_BUF_SIZE];
static uint16_t s_rx_head = 0u;
static uint16_t s_rx_tail = 0u;

static bool debug_serial_rx_pop(uint8_t *out)
{
    ...
    *out = s_rx_buf[s_rx_tail];
    s_rx_tail = (uint16_t)((s_rx_tail + 1u) % DEBUG_SERIAL_RX_BUF_SIZE);
    return true;
}
...
void debug_serial_rx_push(const uint8_t *data, uint32_t len)
{
    ...
    for (uint32_t i = 0u; i < len; i++) {
        ...
        s_rx_head = (uint16_t)((s_rx_head + 1u) % DEBUG_SERIAL_RX_BUF_SIZE);
...
int debug_serial_read(int file, char *ptr, int len, uint8_t type)
{
    ...
    while (debug_serial_rx_pop(&byte)) {
...
int debug_serial_write(int file, char *ptr, int len, uint8_t type)
{
    ...
    else if (host_transport == HOST_TRANSPORT_UART)
    {
        return((HAL_UART_Transmit(&huart1, frame, (uint16_t)frame_len, HAL_MAX_DELAY) == HAL_OK)
               ? len
               : -1);
    }
    else return -1;
