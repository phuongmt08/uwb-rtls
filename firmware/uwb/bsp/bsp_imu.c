...
{
	/* Initialization performs a device soft reset and calibration. Treat it as
	 * a one-time hardware operation so fusion/state resets cannot disturb an
	 * IMU that is already serving samples. */
	if (s_initialized)
	{
		return BSP_IMU_OK;
	}
