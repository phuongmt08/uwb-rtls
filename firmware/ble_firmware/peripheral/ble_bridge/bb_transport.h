 ...
 */
typedef enum {
    BB_SOURCE_UART = 0,     /**< Packet arrived from STM32 via physical UART */
    BB_SOURCE_BLE  = 1,     /**< Packet arrived from Central/Phone via BLE NUS */
} bb_packet_source_t;
 ...
 */
typedef struct {
    bb_packet_source_t source;  /**< Where this packet came from */
    uint8_t *          p_data;  /**< Pointer to the parsed raw payload (without HDLC framing) */
 ...
 */
typedef struct {
    bb_transport_data_handler_t data_handler;  /**< Router callback to trigger upon packet reception */
} bb_transport_init_t;
