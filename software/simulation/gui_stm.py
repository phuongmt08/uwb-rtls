...
from module import config_stm as config
from simulation_stm import run_simulation_stm_with_params

class UKFSimulationGUI_STM(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        # Load file UI — dùng chung simulation.ui
        uic.loadUi(os.path.join(current_dir, "simulation.ui"), self)
        
        # Đổi title để phân biệt
        self.setWindowTitle(self.windowTitle() + " — STM32 Float32")
    ...
    def init_ui_state(self):
        ...
        # CSV Path logic - Đồng bộ với config_stm.py
    ...
    def load_config_to_ui(self):
        """Đổ dữ liệu từ config_stm.py lên giao diện"""
        ...
        self.checkBox_ukfTrapezoidal.setChecked(True)  # Mặc định dùng Trapezoidal (giống C)

    def update_config_var(self, name, value):
        """Cập nhật biến trong config_stm module (in-memory)"""
        try:
    ...
    def update_qr_visibility(self):
        ...
        if is_test:
            # Hiển thị giá trị TEST (Hardcoded giống C)
    ...
    def get_params_for_simulation(self):
        """Thu thập các tham số để truyền vào hàm run_simulation_stm"""
    ...
    def save_to_config_file(self):
        """Lưu các biến hiện tại vào file config_stm.py để persistence"""
        config_path = os.path.join(current_dir, "module", "config_stm.py")
        try:
            ...
            # UKF Constants — Lưu nhưng KHÔNG thay đổi behavior (hardcoded trong C)
            ...
            print(f"Error saving config_stm: {e}")

    def run_simulation(self):
        """Hàm chạy mô phỏng STM32 Float32"""
        ...
        self.pushButton_run.setText("Running (STM32)...")
        ...
        try:
            run_simulation_stm_with_params(params, on_init_parsed=handle_biases)
        except Exception as e:
            QtWidgets.QMessageBox.critical(self, "Lỗi mô phỏng STM32", str(e))
        finally:
