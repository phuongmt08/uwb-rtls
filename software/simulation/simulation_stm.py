"""UKF simulation for UWB + IMU sensor fusion — STM32 Float32 version.

Mirror của simulation.py nhưng dùng module_ukf_stm (float32) + config_stm.
Logic giống hệt sys_sensor_fusion.c.
"""

import numpy as np

import module.config_stm as config
from module.module_csv import find_latest_csv_file, parse_csv_data
from module.module_kinematic import calculate_imu_dead_reckoning_path
from module.module_ukf_stm import create_ukf_context_stm, ukf_predict_stm, ukf_update_stm, init_ukf_logging
from module.module_plot import plot_input_data, plot_position_results
from module.module_txt import UKFLogger
from module.config_stm import IMUSample, ANCHOR_POSITIONS

def run_simulation_stm_with_params(params: dict, on_init_parsed=None) -> None:
    """Run the UWB + IMU UKF simulation with STM32 float32 logic."""
    ...
    config.PROCESS_NOISE_COV = np.diag(np.array([config.Q_A, config.Q_A, config.Q_G], dtype=np.float32))
    config.MEASUREMENT_NOISE_COV = np.diag(np.array([config.R_UWB, config.R_UWB, config.R_UWB], dtype=np.float32))
    ...
    print(f"[STM32 Float32] Dang xu ly file: {config.SOURCE_DATA_FILE}")
    ...
    for name, v_conf in variants.items():
        if v_conf["run"]:
            ctx = create_ukf_context_stm(ukf_initial_state)
            ctx.use_advanced_propagate = params.get("use_advanced_propagate", True)
            if config.OUTPUT_TXT_ENABLED and v_conf["txt"]:
                logger = UKFLogger()
                logger.init_file(config.SOURCE_DATA_FILE, suffix=f"stm_{name}")
                ctx.logger = logger
            contexts[name] = ctx
    ...
    for event in events:
        ...
        if event.type == "Predict":
            ...
            for name, ctx in contexts.items():
                ...
                try:
                    ukf_predict_stm(ctx, imu_sample, event.dt, event.raw_line)
                    ...
                    print(f"\n[STM32][{name}] LOI TAI BUOC PREDICT!")
                    ...
                    print(f"Ngung chay bien the '{name}' tu thoi diem nay.\n")
            ...
            if hasattr(event, 'mask') and getattr(event, 'mask') > 0:
                active_mask = getattr(event, 'mask')
                active_indices = [idx for idx in range(len(d_meas_all)) if (active_mask & (1 << idx)) and d_meas_all[idx] > 1e-6]
            else:
                active_indices = [idx for idx, d in enumerate(d_meas_all) if d > 1e-6]
            
            if len(active_indices) >= 3:
                # Lấy 3 anchor đầu tiên active
                selected_indices = active_indices[:3]
                active_d_meas = d_meas_all[selected_indices]
                
                # Tạo mask cho ukf_update_stm (giống C: bitmask)
                update_mask = 0
                ...
                for name, ctx in contexts.items():
                    try:
                        ukf_update_stm(
                            ctx,
                            active_d_meas[0], active_d_meas[1], active_d_meas[2],
                            update_mask,
                            event.raw_line
                        )
                    except Exception as e:
                        print(f"\n[STM32][{name}] LOI TAI BUOC UPDATE!")
                        print(f"Chi tiet loi: {e}")
        ...
        for name, ctx in contexts.items():
            ukf_results[name]["positions_x"].append(float(ctx.x[0]))
            ukf_results[name]["positions_y"].append(float(ctx.x[1]))
            ukf_results[name]["theta"].append(float(ctx.x[4]))
    ...
    total_time = t
    print(f"\n{'='*60}")
    print(f"[STM32 Float32] THONG KE TAN SO CAP NHAT")
    print(f"{'='*60}")
    ...
    print(f"[STM32 Float32] Bat dau ve bieu do tu {len(events)} mau du lieu CSV...")
...
if __name__ == "__main__":
    print("Vui long chay file gui_stm.py de su dung giao dien.")
