...
Ứng dụng Python + Qt6 để nạp firmware qua USB DFU (STM32 ROM DFU/bootloader DFU), tối ưu cho layout hiện tại:
...
- Nút `Build UWB` để build firmware `firmware/uwb` trực tiếp từ app
...
- Cần có `make` (hoặc `mingw32-make`) trong PATH
- Cần đặt biến môi trường `GCC_PATH` trỏ tới thư mục GNU Arm toolchain (bin), ví dụ: `C:/ST/STM32CubeCLT/.../GNU-tools-for-STM32/bin`
